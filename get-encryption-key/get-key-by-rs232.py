...
def getKeyPart(conn, seq, inverter_id):
    conn.write(struct.pack("<LHHHLLHHH",
        0x79563412, 2, 0xfffd, seq, 0xfffffffd, inverter_id, 18, 0x238 + seq,
        calcCrc(
            struct.pack(">HLLH", seq, 0xfffffffd, inverter_id, 18) +
            struct.pack("<H", 0x238 + seq))))
    conn.flush()
    return conn.read(28)[-8:-4]


connection = serial.Serial(serial_port, 115200)
print("Your key is '" + "".join("\\x%02x" % ord(chr(x)) for i in range(4) for x in
